...
int minRec1(int x[], int n) {
    if(n == 1) return x[n-1];
    if(x[n-1] > x[n-2]) return minRec1(x,n-1);
    else{
        x[n-2] = x[n-1];
...
int minRec2(int x[], int n) {
    if(n==1) return *x;
    if(minRec2(x, n/2) < minRec2(x+(n/2), n/2)) return minRec2(x, n/2);
    else return minRec2(x+(n/2), n/2);
}
...
double sqrtRec(double x, double low_guess, double high_guess) {
    ...
    if((mid*mid)==x)
        ...
        if(mid*mid > x)
            return sqrtRec(x, low_guess, mid-decimal);
        else if(mid*mid < x)
            return sqrtRec(x, mid + decimal, high_guess);
    }
}
...
int strCompare(char* str1, char* str2) {
    if(str1[0] == 0 && str2[0]==0)
        return 0; //both at null at the same time, only true if they were equal
...
bool checkDir(int north, int south, int east, int west){
...
int solveMazeRec(int row, int col) {
    bool pathCheck = path(row,col);
    if(maze[row][col]!=1 && row==MATRIX_SIZE-1){
        maze[row][col]=2;
        return true;
    }
    else if(maze[row][col]==1 || maze[row][col]==2|| !pathCheck){
        return false;
    }
    maze[row][col] = 2;
    ...
    bool checkDirection = checkDir(north, south, east, west);
    if(!checkDirection)
        return true;
